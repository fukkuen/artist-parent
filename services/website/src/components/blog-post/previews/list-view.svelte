...
<a href="blog/{post.metadata.slug}" class="grid grid-cols-3 sm:rounded overflow-hidden group">
	...
	<div class="bg-white group-hover:bg-gray-100 col-span-2 py-4 px-2 sm:p-8 flex w-full flex-col">
		<h3 class="text-p2 sm:text-p3 leading-tight mb-1 sm:mb-4">{post.metadata[`title_${$locale}`]}</h3>
		<div class="flex items-center mt-auto">
			<p class="text-sm sm:text flex-1 font-bold text-gray-500">{post.metadata.record_no}. {post.metadata[`artist_name_${$locale}`]}</p>
