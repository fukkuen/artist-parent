...
<a href="blog/{post.metadata.slug}" class="grid grid-cols-3 sm:rounded overflow-hidden group">
	...
	<div class="bg-orange-400 group-hover:bg-orange-500 col-span-2 py-4 px-2 sm:p-8 flex w-full flex-col">
		<h3 class="text-white text-p2 sm:text-p3 leading-tight mb-1 sm:mb-4">{post.metadata[`title_${$locale}`]}</h3>
		<div class="flex items-center mt-auto">
			<p class="text-sm sm:text flex-1 font-bold text-orange-500 group-hover:text-orange-300">{post.metadata.record_no}. {post.metadata[`artist_name_${$locale}`]}</p>
