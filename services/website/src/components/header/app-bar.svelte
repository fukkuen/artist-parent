<script>
	...
	let hamburger_mount = false
	import {onMount} from 'svelte'
	onMount(() => {
		hamburger_mount = true
	})
...
<div class="{off_screen ? 'translate-y-20' : 'translate-y-0'} transform transition-transform fixed inset-x-0 bottom-0 bg-white shadow z-40 block sm:hidden py-1">
	...
	<div class="flex justify-around items-center relative z-10 bg-white text-gray-500">
		<a href="/" class="w-8 h-8 flex justify-center items-center rounded-full {$page.path === '/blog/category/all/1' ? 'bg-gray-200 text-red-500' : ''}">
		...
		<a href="/bookmark" class="w-8 h-8 flex justify-center items-center rounded-full {$page.path === '/bookmark' ? 'bg-gray-200 text-red-500' : ''}">
			<Icon className="w-4" name="love"/>
		</a>
		<button on:click|stopPropagation={() => {menu_on = !menu_on}} class="w-8 h-8" style="max-width: 2em; max-height: 2em">
			{#if hamburger_mount}
				<Ham open={menu_on}/>
			{/if}
		</button>
