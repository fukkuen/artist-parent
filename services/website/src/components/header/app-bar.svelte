...
<div class="fixed inset-x-0 bottom-0 bg-white shadow z-40 block sm:hidden">
	...
	<div class="grid grid-cols-3 relative z-10 bg-white">
		<a href="/" class="w-10 h-10 flex justify-center items-center mx-auto rounded-full" class:bg-gray-200={$page.path === '/blog/category/all/1'}>
		...
		<a href="/bookmark" class="w-10 h-10 flex justify-center items-center mx-auto rounded-full" class:bg-gray-200={$page.path === '/bookmark'}>
			<Icon className="w-4" name="love"/>
		</a>
		<button on:click|stopPropagation={() => {menu_on = !menu_on}} class="w-10 h-10 flex justify-center items-center mx-auto transform scale-75">
			<Ham open={menu_on}/>
		</button>
	</div>
</div>