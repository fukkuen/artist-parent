...
<nav class="text-gray-500 block sm:hidden fixed inset-x-0 top-0 z-50 transition-colors" class:bg-white={floated} class:shadow-lg={floated}>
	<div class="max-w-screen-xl mx-auto h-10">
		<button on:click={() => {history.back()}} class="h-10 w-10 flex items-center justify-center rounded-full" class:text-gray-500={floated}>
